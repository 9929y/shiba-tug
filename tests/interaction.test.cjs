...
test('short, comfortable and excessive pulls yield different results',()=>{assert.equal(previewPull({tension:.07}).gain,0);assert.ok(previewPull({tension:.5}).gain>previewPull({tension:.15}).gain);assert.ok(previewPull({tension:.8}).gain<previewPull({tension:.7}).gain);assert.equal(previewPull({tension:.9}).gain,.08)});
test('backoff rewards and recovery after 150ms',()=>{let p={tension:0,peak:0,sampleTime:0,valid:false};samplePull(p,1,1000);assert.equal(previewPull(p).gain,.08);samplePull(p,.5,1100);samplePull(p,.5,1249);assert.equal(p.blocked,true);samplePull(p,.5,1250);assert.equal(p.blocked,false);assert.ok(previewPull(p).gain>previewPull({tension:.5}).gain)});
...
test('overpull is blocked, hold backoff recovers',async()=>{const g=await game();g.send('pointerdown');g.send('pointermove',370);assert.equal(+g.get('stage').dataset.preview,.08);g.send('pointermove',490);await g.tick(170);assert.ok(+g.get('stage').dataset.preview>0);g.send('pointerup',490);await g.tick(1000);assert.ok(+g.get('stage').dataset.progress>0)});
...
test('repeated overpulls make visible progress and finish for either breed',async()=>{for(const breed of ['black','yellow']){const g=await game();if(breed==='black'){g.get('black').events.click();await g.tick(100)}for(let i=0;i<13;i++){g.send('pointerdown');g.send('pointermove',350);assert.ok(+g.get('stage').dataset.preview>0);g.send('pointerup',350);await g.tick(1200);assert.ok(+g.get('stage').dataset.progress>i*.075);}await g.tick(6000);assert.equal(g.get('stage').dataset.state,'complete');assert.match(g.get('routeValue').textContent,/100%/);}});
